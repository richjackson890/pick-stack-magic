...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Check if display_name is set in profiles
  const checkProfileComplete = async (uid: string) => {
    try {
      const { data } = await (supabase
        ...
        .eq('id', uid)
        .maybeSingle() as any);
      setProfileComplete(!!data?.display_name && data.display_name.trim() !== '');
    } catch {
    ...
    try {
      ...
      if (error) {
        if (error.message.includes('already registered')) {
          return { error: new Error('이미 가입된 이메일입니다. 로그인해 주세요.') };
        }
        return { error };
      }
  ...
  const signInWithGoogle = async () => {
    try {
      // Redirect to /auth so pending invite token check runs after OAuth
      ...
      if (error) {
        if (error.message.includes('redirect_uri_mismatch')) {
          return { error: new Error('리디렉션 URL이 일치하지 않습니다. 관리자에게 문의하세요.') };
        }
        return { error };
      }
